...
Closure of the multi-stack harness and the `android` profile. Major news since
`v1.0-internal`: new **Android** profile (Kotlin/Gradle); **profile-aware Role
Guard** (authorizes `app/`+Gradle for android and the feature documentation
subtrees for product); **mutation-testing finalizable** end-to-end (report
folded into the single QA commit); and the ECC/agency-agents adoptions
(eval-harness, prompt-defense, stale-replay guard, iterative-retrieval,
tool-telemetry, QA pre-report gate, report templates). Validated in a real
pilot (PokeCards projects: Android app + scraping backend, both at 100% of
features). CI green.

### Added

- Capability `eval-harness`: converts each feature's `SCN-XXX` scenarios into
  executable graders (`code`/`rule` eligible for gate; `model`/`human`
  advisory) with `pass_at_k`/`pass_caret_k` metrics. Runner
  `scripts/run_evals.py`, validator, schema and policy; gate `EVAL-001` in
  `qa_full`. Decision in `docs/adr-0002-eval-harness-verification-gate.md`.
  Adopted from `affaan-m/ECC`.
- Capability `tool-telemetry`: `PreToolUse`/`PostToolUse` hooks record each
  tool call as JSONL with secret scrubbing
  (`scripts/tool_telemetry_hook.py`); no-op and fail-soft without the capability.
- QA report in Markdown with fixed fields (`evidence/reviews/<feature>.md`)
  alongside the JSON, generated by `scripts/complete_review.py` (report template).
- Agent hardening (adopted from ECC / agency-agents): anti-injection "Prompt
  defense" block in all 7 agents; `qa-reviewer` with a default bias toward
  `CHANGES_REQUESTED`, automatic-failure triggers and a pre-report gate;
  stale-replay guard in `leader`/`implementer`; iterative context recovery in
  `architect`/`implementer`.
- QA retry cap with escalation: `complete_review.py` counts the
  `CHANGES_REQUESTED` in `qa_attempts`; `start_implementation.py` rejects a new
  attempt when `maximum_qa_attempts` is reached (default 3 in
  `state/project.json`) and the `leader` escalates to a human decision. Prevents
  infinite QA<->implementer loops (adopted from agency-agents handoff-templates).
- `implementer` with minimal-change discipline and a Scope Self-Check (each line
  of the diff justifiable by an `AC-XXX`, no scope creep; adopted from
  agency-agents minimal-change-engineer).
- `specifier` with a "Capability contract" section (observable promise vs
  implementation, invariants, states/transitions, uncertainty as `Q-XXX`,
  non-goals; adopted from ECC product-capability).

### Changed

- `finalize_feature.py` accepts the QA Markdown report alongside the JSON in the
  evidence commit following the reviewed commit.
